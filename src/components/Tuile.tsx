...
export default function Tuile({ valeur, unite, fond, inclinaison }: Props) {
  return (
    ...
    >
      <span className="block h-[1.1em] overflow-hidden font-titre text-6xl leading-none sm:text-7xl md:text-8xl [@media(max-height:500px)]:text-6xl">
        <span key={valeur} className="inline-block animate-pop">
          {String(valeur).padStart(2, "0")}
        </span>
      </span>
      <span className="mt-2 text-sm font-bold uppercase tracking-widest sm:text-base">{unite}</span>
    </div>
  );
}
